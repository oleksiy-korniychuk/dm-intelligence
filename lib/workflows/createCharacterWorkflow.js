import { buildUpContext, generateGmResponse } from "@/lib/llm/createCharacterService";
...
async function fetchAdditionalInfo(state) {
    ...
    try {
        const result = await buildUpContext(state.history, state.character, state.userId);
...
async function generateResponse(state) {
    ...
    try {
        const response = await generateGmResponse(state.history, state.character, state.userId);
...
export async function processPlayerMessage(sessionId, messages, character, userId = null) {
    try {
