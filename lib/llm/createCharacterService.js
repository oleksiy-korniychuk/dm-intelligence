import { characterChatSchema } from "@/lib/schemas/characterChatSchema.js";
...
function convertHistoryToMessages(history) {
    const messages = [];
    const pendingToolCallIdsByName = {};
    ...
    for (const msg of history) {
        ...
        if (msg.parts) {
            ...
            // Function call from model
                ...
                if (!pendingToolCallIdsByName[functionName]) {
                    pendingToolCallIdsByName[functionName] = [];
                }
                pendingToolCallIdsByName[functionName].push(toolCallId);
            }
            // Function response
            else if (part.functionResponse) {
                const functionName = part.functionResponse.name;
                if (!functionName || !pendingToolCallIdsByName[functionName]?.length) {
                    throw new Error("Missing matching tool_call_id for function response");
                }
                const toolCallId = pendingToolCallIdsByName[functionName].pop();
